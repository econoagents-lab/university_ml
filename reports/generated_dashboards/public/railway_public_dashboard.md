...
**Familia:** `public`  
**Owner:** Producto / Demo  
**Audiencia:** Externo seguro  
**Prioridad:** mvp  
**Estado:** cataloged

## Pregunta económica

¿Qué puede ver alguien externo sin PII?
...
| `status` | ok |
| `data_mode` | crm |
| `total_operaciones_publicas` | 763 |
| `campos_publicos` | ["data_mode", "fecha_generacion", "p0_p1", "riesgo_promedio", "top_asesores", "top_canales", "top_proyectos", "total_operaciones", "valor_total_en_riesgo"] |
| `privacy_errors` | [] |
| `decision` | Yo sirvo solo payload público agregado en Railway y bloqueo fallback demo en producción. |

## Acción recomendada

Servir solo agregados públicos; bloquear sample fallback y cualquier campo sensible.

## Donde cambiar

`config/privacy_policy.yml#public_dashboard`

## Parámetros actuales usados como contexto

```yaml
expose_project_names: true
anonymize_advisors_public: true
expose_channel_names: true
max_top_n: 5
data_mode_required: crm
